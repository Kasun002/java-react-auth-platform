...
/**
 * Admin API — RBAC management endpoints for permissions, roles, groups, and user membership.
 *
 * <p>All endpoints require a valid ACCESS JWT. Fine-grained authority checks are enforced
 * via {@code @PreAuthorize}. Any request without sufficient authority returns 403.</p>
 *
 * <p>Banking standards: PCI-DSS v4 Req 7.2 (least privilege), ISO 27001 A.9.4.2
 * (separation of duties). All admin actions are logged via MDC correlation ID.</p>
 */
@Slf4j
...
public class AdminController {
    ...
    @Operation(summary = "Assign permission to role",
            description = "Adds a permission to a role. Idempotent — no error if already assigned.")
    @ApiResponses({
    ...
    @Operation(summary = "Remove permission from role",
            description = "Removes a permission from a role. No-op if not currently assigned.")
    @ApiResponses({
    ...
    @Operation(summary = "Assign role to group",
            description = "Adds a banking role to a group. Idempotent — no error if already assigned.")
    @ApiResponses({
    ...
    @Operation(summary = "Remove role from group",
            description = "Removes a role from a group. No-op if the role is not assigned.")
    @ApiResponses({
    ...
    @Operation(summary = "Get user group memberships",
    ...
    @Operation(summary = "Add user to group",
            description = "Assigns a user to a group. Idempotent — no error if already a member.")
    @ApiResponses({
    ...
    @Operation(summary = "Remove user from group",
            description = "Removes a user from a group. No-op if the user is not a member.")
    @ApiResponses({
    ...
    @Operation(summary = "Get user effective permissions",
            description = "Returns the complete set of permission codes a user holds — "
