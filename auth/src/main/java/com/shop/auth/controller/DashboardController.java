 ...
 *
 * <p>All data is read-only. A single {@code GET /admin/dashboard/stats} call
 * returns KPI counts, user-status breakdowns, group distributions, permission
 * category totals, and recent login activity in one round-trip.</p>
 *
 * <p>Banking standards: PCI-DSS v4 Req 7.2 (least-privilege access review),
 * Req 10.2 (audit log visibility). Requires {@code DASHBOARD_VIEW} authority.</p>
 */
@Slf4j
...
public class DashboardController {
    ...
    @Operation(
        ...
        description = "Returns a single aggregated snapshot: KPI counts (users/groups/roles/permissions), "
    ...
    @ApiResponses({
        ...
        @ApiResponse(responseCode = "403", description = "Insufficient authority — DASHBOARD_VIEW required",
            content = @Content(schema = @Schema(implementation = ResponseDto.class)))
    })
    @GetMapping("/stats")
