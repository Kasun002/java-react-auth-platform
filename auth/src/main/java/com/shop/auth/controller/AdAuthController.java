 ...
 *
 * <p>The client (SPA or mobile app) authenticates with Azure AD using MSAL / PKCE,
 * receives an OIDC ID token, and exchanges it here for this service's own JWT pair.
 * The AD ID token is never forwarded to downstream services — only this service's
...
public class AdAuthController {
    ...
    @Operation(
        summary     = "Exchange an Azure AD ID token for a service JWT pair",
        description = "Validates the OIDC ID token issued by Azure AD (or Keycloak in dev), " +
