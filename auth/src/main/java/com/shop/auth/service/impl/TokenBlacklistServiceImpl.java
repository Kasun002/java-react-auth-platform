 ...
 *
 * <p>Two revocation mechanisms:</p>
 * <ol>
 *   <li><b>Per-token blacklist</b> — key {@code blacklist:jti:<jti>}, value {@code "1"}, TTL = remaining token lifetime.
 *       Used for individual logout and refresh token rotation.</li>
 *   <li><b>User-level invalidation</b> — key {@code user:tokens:invalidated:<userId>}, value = epoch seconds of invalidation,
 *       TTL = refresh token lifetime. Used on password change, account suspension, and admin-forced logout.
 *       Any token whose {@code iat} is older than this timestamp is rejected.</li>
 * </ol>
 *
 * <p>All Redis operations are O(1) — negligible per-request overhead.</p>
 */
@Slf4j
@Service
