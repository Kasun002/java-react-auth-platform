...
Production Banking Assessment — MQ, Kafka, Caching & Gaps

  ---
  1. Redis — Critical. Non-negotiable.

  Token Revocation / Blacklist

  The biggest production gap right now. The JwtAuthenticationFilter only checks signature and expiry. It never checks
  whether a token has been revoked. This means:
  - A user who logs out, changes password, or gets their account suspended still holds a valid token until it expires
  (up to 15 min)
  - In banking, 15 minutes of lingering access after compromise is unacceptable

  Redis is the standard solution. On logout or password change, the token's jti (already in the JWT) gets written to a
   Redis set with a TTL matching the token's remaining lifetime. The filter checks this set on every request. The
  check is a single O(1) Redis lookup — negligible latency.

  Distributed Brute-Force / Rate Limiting

  Current implementation stores failedLoginAttempts in the DB on the users row. This works correctly for a single
  instance but in a multi-pod Kubernetes deployment, the counter is fine since it's DB-persisted. However, the OTP
  rate limiting (countByUserAndCreatedAtAfter) fires a DB query on every verify/resend attempt. Under load this adds
  latency and DB pressure. Redis counters with INCR + EXPIRE are atomic, sub-millisecond, and designed for this. Use
  Redis for all rate limiting counters: login attempts, OTP verify attempts, OTP resend counts, and future endpoints
  like /change-password.

  Permission / RBAC Cache

  Currently permissions are computed at login time and embedded in the JWT — that's good. But AdminController
  operations (assign/remove permissions, add/remove groups) don't invalidate anything because the JWT is a snapshot.
  If an admin revokes a permission from a user mid-session, nothing happens until the next login. For a 15-minute
  access token this is acceptable per PCI-DSS 8.3.9. But if you ever extend token lifetime or need instant revocation
  (e.g., a compromised account), you need Redis to cache and invalidate the effective permission set per userId, and
  the filter should optionally consult it.

  Summary: Redis is the first infrastructure addition you need. It covers token blacklist, distributed rate limiting,
  and optional permission cache.

  ---
  2. Message Queue (RabbitMQ or AWS SQS) — Strongly recommended.

  OTP Email Delivery

  Already noted in the implementation gaps, but worth reinforcing the production severity. Today
  emailService.sendOtp() is synchronous — if the SMTP server takes 3 seconds, the /register and /resend-otp HTTP
  responses take 3 seconds. Under load or when the mail server is slow/down:
  - Registration requests queue up, threads block, Tomcat thread pool exhausts
  - A single SMTP outage can take down the entire registration flow

  The correct pattern: the service writes a message to a queue ({userId, email, name, otp, expiry}) and returns
  immediately. A separate email-worker service consumes the queue and handles retries, dead-lettering, and delivery
  receipts. The auth service becomes fast and decoupled from mail infrastructure.

  Audit Event Publishing

  Banking requires every security-relevant event to be shipped to a SIEM (Splunk, Elastic SIEM, etc.) in near
  real-time: login success, login failure, OTP issued, OTP verified, password changed, account locked, permission
  changed, group assignment changed. Currently all of this is either in application logs or the user_log table. That's
   not sufficient for a banking SIEM — you need a structured, queryable, immutable event stream. A queue (or Kafka)
  bridges the auth service to the SIEM without blocking the request path.

  Future change-password / reset-password flows

  Reset-password will need to send an email link. Same pattern applies — async via queue.

  ---
  3. Kafka — For enterprise scale / fraud detection. Not required at first.

  Kafka is not needed for the auth module to function correctly. It becomes relevant when:

  - Fraud detection: A downstream fraud-detection service needs a real-time feed of login events (IP, device
  fingerprint, geo-location, time-of-day) to score risk. Kafka is the right transport for that feed.
  - Compliance streaming: Regulators may require an immutable, replayable audit log. Kafka topics with log compaction
  serve this.
  - Cross-service token revocation: If you have multiple microservices that also validate JWTs independently (not just
   this auth service), broadcasting a revocation event via Kafka ensures every service drops the token from its local
  Redis cache simultaneously.

  For the current scope (single auth microservice + future AD/OIDC), start with RabbitMQ/SQS for async email and audit
   events. Add Kafka when you build the fraud detection layer.

  ---
  4. Gaps Not Yet Mentioned in IMPLEMENTATION.md

  A. No Logout Endpoint
  /auth/logout does not exist. Without it, there is no way to explicitly invalidate a token. Users closing the app
  just discard the token client-side — the server-side token remains valid until expiry. This is a hard requirement in
   banking. Logout must accept the access token, extract its jti, and write it to the Redis blacklist. The refresh
  token jti must also be blacklisted.

  B. Token Revocation Not Checked in the Filter
  Even if a user_log hash exists, JwtAuthenticationFilter never consults it. The filter only checks signature and
  expiry. There is no server-side revocation path today for any reason — logout, password change, account suspension,
  or admin action.

  C. Password Policy is Critically Weak
  The only password constraint is @Size(min = 8). Banking standards require:
  - Uppercase + lowercase + digit + special character (NIST 800-63B, PCI-DSS Req 8.3.6)
  - Password history — cannot reuse the last N passwords (typically 12 in banking)
  - Maximum password age — force rotation every 90 days (PCI-DSS Req 8.3.9)
  - Breached password check — optionally screen against HaveIBeenPwned API

  D. No Security Response Headers
  SecurityConfig sets no HTTP security headers. For a banking API these are required:
  - Strict-Transport-Security (HSTS) — forces HTTPS
  - X-Content-Type-Options: nosniff
  - X-Frame-Options: DENY
  - Cache-Control: no-store — prevents browser caching of auth responses
  - Content-Security-Policy

  Spring Security provides headers() configuration for all of these.

  E. No CORS Policy
  SecurityConfig does not configure CORS. In production, no CORS headers means either everything is blocked by
  browsers (if the frontend is on a different origin) or you're relying on the framework default. For a banking API,
  CORS should be explicitly set to the specific allowed origin(s) — never a wildcard.

  F. No IP Address / Device Info in Audit Log
  The user_log table only stores a token hash, type, issued/expiry times. It stores nothing about where the login came
   from. Banking requires logging:
  - IP address
  - User-Agent string (device/browser fingerprint)
  - Login timestamp and outcome

  This is required by PCI-DSS Req 10.2.4 and 10.2.7, and is fundamental to detecting account takeover (login from a
  new country, login from a known-malicious IP, simultaneous logins from different continents).

  G. No Refresh Token Rotation
  When a refresh token is used to obtain a new access token (once that endpoint is built), the old refresh token
  should be immediately invalidated. Without rotation, a stolen refresh token can be used indefinitely for 7 days.
  Banking standard is single-use refresh tokens with rotation.

  H. No Concurrent Session Control
  A user can log in from unlimited devices simultaneously, each holding their own valid token pair. Banking typically
  enforces a maximum concurrent session count (e.g., 3 active refresh tokens at once). Exceeding the limit should
  revoke the oldest session. This requires a per-user session registry — Redis is the natural store.

  I. JWT Secret Not in a Secret Manager / HSM
  The JWT signing secret is an env var string. In a banking production environment, signing keys should be stored in
  HashiCorp Vault, AWS KMS, or an HSM, with automatic rotation. The JwtServiceImpl should load the key from Vault at
  startup, not from a plain application property.

  J. No JWT Key Rotation Strategy
  If the JWT secret is compromised or due for rotation, there is currently no mechanism to rotate it without
  invalidating every active token simultaneously (instant global logout for all users). A standard solution is key
  versioning: maintain a kid (key ID) claim in the JWT header, keep N previous keys valid during the rotation window,
  and retire them after all tokens signed with the old key have expired.

  K. SQL and TRACE logging enabled at production level
  application.properties has logging.level.org.hibernate.SQL=DEBUG and
  logging.level.org.hibernate.orm.jdbc.bind=TRACE. These log every SQL statement and every bind parameter value to the
   application log. In production this means passwords during registration (even pre-hash) and OTP hashes could appear
   in logs depending on which queries are traced. These must be OFF or WARN in production. A profile-based
  application-prod.properties should override these.

  L. No change-password Endpoint
  /auth/change-password (requires current password + new password) is not implemented. Until it exists, users have no
  way to change their password. Crucially, password change must also invalidate all existing tokens for that user —
  both the current access and refresh tokens. This requires the Redis blacklist.

  M. No reset-password Flow
  /auth/forgot-password + /auth/reset-password (email link with a time-limited signed token, separate from the JWT) is
   absent. The reset token needs its own storage (Redis with short TTL, e.g., 15 minutes) and must be single-use.

  N. AD / OIDC Integration Preparation

  When you add Active Directory (LDAP) or OIDC (Azure AD, Okta, Google):
  - There is currently no UserDetailsService bean — Spring Security's OIDC/LDAP flows need one to map external
  identities to internal User records
  - There is no ExternalIdentity entity or table — you'll need to link an external sub (OIDC subject) or
  sAMAccountName (AD) to your internal users.id
  - RBAC group mapping: AD groups need to map to your UserGroup entities. This mapping table doesn't exist yet
  - PKCE (code_challenge) support is needed for the OIDC authorization code flow — Spring Security OAuth2 Client
  handles this, but it requires separate configuration in SecurityConfig
  - SSO logout (back-channel logout from the IdP) needs to be handled — when the IdP invalidates a session, your
  service needs to be notified and blacklist the corresponding tokens
  ...
  ├──────────┼──────────────────────────────────────┼─────────────────────────────────────────────────────────────┤
  │ 1        │ Redis                                │ Token blacklist + distributed rate limiting. Unblocks       │
  │          │                                      │ logout and everything that follows                          │
  ├──────────┼──────────────────────────────────────┼─────────────────────────────────────────────────────────────┤
  │ 2        │ /auth/logout                         │ Requires Redis. First concrete use of the blacklist         │
  ├──────────┼──────────────────────────────────────┼─────────────────────────────────────────────────────────────┤
  │ 3        │ /auth/change-password                │ Requires Redis for token invalidation on success            │
  ├──────────┼──────────────────────────────────────┼─────────────────────────────────────────────────────────────┤
  │ 4        │ /auth/forgot-password +              │ Requires Redis for reset token storage                      │
  │          │ /auth/reset-password                 │                                                             │
  ├──────────┼──────────────────────────────────────┼─────────────────────────────────────────────────────────────┤
  │ 5        │ Password policy enforcement          │ Independent. Add a PasswordPolicyService                    │
  ├──────────┼──────────────────────────────────────┼─────────────────────────────────────────────────────────────┤
  │ 6        │ Security response headers + CORS     │ Config-only change in SecurityConfig                        │
  ├──────────┼──────────────────────────────────────┼─────────────────────────────────────────────────────────────┤
  │ 7        │ IP/device logging in user_log        │ Schema + entity change                                      │
  ├──────────┼──────────────────────────────────────┼─────────────────────────────────────────────────────────────┤
  │ 8        │ Async email via MQ                   │ Decouple email delivery from request path                   │
  ├──────────┼──────────────────────────────────────┼─────────────────────────────────────────────────────────────┤
  │ 9        │ Refresh token rotation + concurrent  │ Requires Redis session registry                             │
  │          │ session limit                        │                                                             │
  ├──────────┼──────────────────────────────────────┼─────────────────────────────────────────────────────────────┤
  │ 10       │ Vault/KMS for JWT key                │ Infra change                                                │
  ├──────────┼──────────────────────────────────────┼─────────────────────────────────────────────────────────────┤   
  │ 11       │ OIDC / AD login                      │ Requires all of the above plus ExternalIdentity entity      │
  ├──────────┼──────────────────────────────────────┼─────────────────────────────────────────────────────────────┤   
  │ 12       │ Kafka                                │ When fraud detection / SIEM streaming is in scope           │
  └──────────┴──────────────────────────────────────┴─────────────────────────────────────────────────────────────┘