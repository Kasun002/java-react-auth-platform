 ...
 *
 * <p>
 * Held in the {@code SecurityContext} for the duration of the request.
 * Controllers and services can access it via
 * {@code (UserPrincipal) SecurityContextHolder.getContext().getAuthentication().getPrincipal()}.
 * </p>
 *
 * <p>
 * Account-level flags (locked, enabled, etc.) are intentionally {@code true}:
 * those checks are performed at login time. A valid JWT implies the account was
 * ACTIVE when the token was issued.
 * </p>
 */
